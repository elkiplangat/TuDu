    ...
    override fun onCreate(savedInstanceState: Bundle?) {
      ...
      // viewModel = ViewModelProviders.of(this, ToDoActivitiesFragment.viewModelFactory ).get(ToDoActivitiesViewModel::class.java)
        tvdatepicker.setOnClickListener{
            ...
            material_datepicker.init(today.get(Calendar.YEAR), today.get(Calendar.MONTH), today.get(Calendar.DAY_OF_MONTH) ){
                    view, year, month, day ->
                val month = month + 1
                ...
                saveDate =  date.date.toString() + "/" + date.month.toString() + "/" + date.year.toString()
           ...
           title = etTitle.text.toString().trim()
            ...
            if (title.isNullOrEmpty()||activity.isNullOrEmpty()){
               // Toast.makeText(applicationContext, "Clicked!", Toast.LENGTH_LONG).show()
                Snackbar.make(findViewById(R.id.constraintLayoutMain), "Fields Cant be empty", Snackbar.LENGTH_LONG).show()
            }
            else{
